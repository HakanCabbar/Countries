...
import { DetailRow } from 'src/components/country-details/detail-row'
...
const CountryDetails = () => {
  ...
  const { data: countryData } = useGetCountryDetails({
    filterValue: countryName as string,
    fields:
      'altSpellings,name,capital,population,languages,currencies,continents,region,subregion,area,landlocked,borders,maps,flags,coatOfArms,unMember,independent,timezones,startOfWeek,tld,postalCode'
  })
          ...
          <Box key={index}>
            {index === generalInformationData.length - 1 ? (
              <DetailRow key={index} label={data.label} isLastMember>
                <Chip color='blue' text={data.value} />
              </DetailRow>
            ) : (
